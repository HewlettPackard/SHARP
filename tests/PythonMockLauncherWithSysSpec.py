...
class PythonMockLauncherWithSysSpec(Launcher):
    ...
    def sys_spec_commands(self) -> Dict[str, str]:
        """Map all sys specs to a dummy string.

        Returns:
            Dictionary mapping system specs to mock commands
        """
        return { spec: f"/bin/echo Python mock sysspec: {cmd}" for spec, cmd in self._sys_spec.items() }
