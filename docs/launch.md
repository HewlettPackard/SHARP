...
### Discovering Available Repeaters

To see all available repeater strategies and their descriptions, use:

```bash
launch --list-repeaters
```

This displays a table of all repeaters with their one-line descriptions. Example output:
...
Name                 Description
--------------------------------------------------------------------------------
BB                   Stopping rule for autocorrelated series based on Block...
CI                   Stop repeating when the 95% right-tailed confidence in...
COUNT / MAX          Simple Repeater that stops after predetermined number ...
DC                   Meta-heuristic stopping rule
GMM                  Gaussian Mixture stopping rule
HDI                  Stop when Highest Density Interval width drops below...
KS                   Stop when the Kolmogorov-Smirnov statistic between the...
RSE                  Relative Standard Error stopping rule
```

To see detailed configuration options for a specific repeater, use:

```bash
launch --show-repeater RSE
```

This displays the repeater's full description, all configurable parameters with their types, defaults, and help text.

**Note:** Discovery flags (`--list-repeaters`, `--show-repeater`) are mutually exclusive with experiment execution options. Use them separately to explore available options, then configure your experiment.
...
Note that the `metric` parameter is at the top level of `repeater_options` (not inside the repeater-specific sub-dictionary like "HDI" or "RSE") because it's a shared parameter inherited by all repeaters from the base CountRepeater class.

All repeaters support the following three configuration options:

 * `max`: Maximum number of repetitions, regardless of other criteria (repeater-specific, placed in repeater's sub-dict).
 * `verbose`: Controls whether to print extra information to elucidate why the algorithm stops when it does (top-level in repeater_options).
 * `metric`: The name of the performance metric that needs to converge (default: `outer_time`). This is a **shared parameter** placed at the top level of `repeater_options`, not in a specific repeater's sub-dictionary.

The full list of available repeaters and their options is listed next, using their short name as used in the `-r` argument.
For complete algorithm documentation, refer to the source code (`src/core/repeaters/` directory) or use `--show-repeater`.
